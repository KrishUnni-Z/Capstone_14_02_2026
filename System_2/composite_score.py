...
# Actual p24 dimension values from held-out meta learner predictions
if full_preds_df is not None and "period_id" in full_preds_df.columns:
    actual_dims_p24 = full_preds_df[full_preds_df["period_id"] == 24][
        ["goal_id", "coherence", "relevance", "integrity", "attainability"]
    ].copy()

    actual_dims_p24 = actual_dims_p24.rename(columns={
    ...
    proj = proj.merge(actual_dims_p24, on="goal_id", how="left")

    proj["coherence_error_p24"] = (
        proj["coherence_p6"] - proj["actual_coherence_p24"]
    ).abs()

    proj["relevance_error_p24"] = (
        proj["relevance_p6"] - proj["actual_relevance_p24"]
    ).abs()

    proj["integrity_error_p24"] = (
        proj["integrity_p6"] - proj["actual_integrity_p24"]
    ).abs()

    proj["attainability_error_p24"] = (
        proj["attain_p6"] - proj["actual_attainability_p24"]
    ).abs()
...
save_cols = [
    ...
    "coherence_slope", "relevance_slope", "integrity_slope", "attain_slope",
]
