...
def build_gp_kernel():
    """Build a fresh temporal GP kernel (Matern x RBF) for any dimension."""
    k_feat = ConstantKernel(1.0, (0.1, 10.0)) * Matern(
        length_scale=1.0, length_scale_bounds=(0.5, 20.0), nu=1.5
    )
    k_time = RBF(
    ...
    if PERIOD_FEAT in feat_cols:
        return SlicedProduct(k_feat, k_time, feature_idx, period_idx) + WhiteKernel(
            noise_level=0.05, noise_level_bounds=(1e-5, 0.5)
        )
    return (
...
# ══════════════════════════════════════════════════════════════════════════════
# ENGINEERED SCORES — R / C / I  (baseline for GP residuals)
# ══════════════════════════════════════════════════════════════════════════════
print("\n" + "─" * 60)
...
# ══════════════════════════════════════════════════════════════════════════════
# MULTI-DIM GPs — Coherence, Relevance, Integrity
# Each GP trains on (LLM_score - engineered_score) residuals at p6/p12/p18.
# Prediction = engineered_baseline + GP_residual, confidence-blended.
# This keeps engineered values as grounding while the GP learns what rules miss.
# ══════════════════════════════════════════════════════════════════════════════
print("\n" + "─" * 60)
print("MULTI-DIM GPs — C / R / I trained on LLM residuals from engineered baseline")
print("─" * 60)

# Build snapshot training data (p6/p12/p18) — features + LLM scores
_snap_feat = features_full[features_full["period_id"].isin([6, 12, 18])].copy().reset_index(drop=True)
_snap_llm  = llm_preds[llm_preds["period_id"].isin([6, 12, 18])].copy().reset_index(drop=True)

_snap = _snap_feat.merge(
    _snap_llm[[c for c in ["goal_id", "period_id"] + [
        f"{m}_{d}" for m in MODELS for d in ["coherence", "relevance", "integrity"]
        if f"{m}_{d}" in _snap_llm.columns
    ] + [f"{m}_success" for m in MODELS if f"{m}_success" in _snap_llm.columns]]],
    on=["goal_id", "period_id"], how="inner"
).reset_index(drop=True)

X_snap = _snap[feat_cols].values

# Engineered baselines at snapshots
_eng_at_snap = {
    "relevance" : engineered_relevance(_snap),
    "coherence" : engineered_coherence(_snap),
    "integrity" : engineered_integrity(_snap),
}

dim_gps = {"attainability": gp}  # attainability GP already trained

# Engineered baselines for ALL 840 rows (used as weak prior for non-snapshot rows)
_eng_full_rel = engineered_relevance(features_full)
_eng_full_coh = engineered_coherence(features_full)
_eng_full_int = engineered_integrity(features_full)
_eng_full = {
    "relevance": _eng_full_rel,
    "coherence": _eng_full_coh,
    "integrity": _eng_full_int,
}

# Snapshot mask over all 840 rows
_snap_mask_full = features_full["period_id"].isin([6, 12, 18]).values

# Heteroscedastic noise: LLM-labelled rows trusted strongly, engineered-only rows
# treated as a weak prior (residual = 0, high noise = regulariser toward baseline).
# This gives the GP 840 training rows instead of 105 while preserving LLM as ground truth.
NOISE_LLM = 1e-4    # tight: trust the LLM label
NOISE_ENG = 0.30    # loose: engineered-only rows just regularise toward zero residual

for dim in ["coherence", "relevance", "integrity"]:
    # ── Step 1: LLM scores at snapshot periods ───────────────────────────────
    llm_sum   = np.zeros(len(_snap))
    llm_count = np.zeros(len(_snap))
    for m in MODELS:
        dcol = f"{m}_{dim}"
        scol = f"{m}_success"
        if dcol not in _snap.columns or scol not in _snap.columns:
            continue
        ok = _snap[scol].fillna(False).values.astype(bool)
        llm_sum[ok]   += _snap.loc[ok, dcol].values.astype(float)
        llm_count[ok] += 1

    valid_snap = llm_count > 0
    y_llm_snap = np.where(valid_snap,
                          llm_sum / np.maximum(llm_count, 1),
                          _eng_at_snap[dim])

    # ── Step 2: Build full 840-row training set ───────────────────────────────
    # y_resid = 0 everywhere (engineered = target), then override at snapshots
    eng_full   = _eng_full[dim]
    y_resid_full = np.zeros(len(features_full))   # non-snapshot rows: residual = 0

    # Fill residuals at snapshot rows from LLM labels
    snap_idx = np.where(_snap_mask_full)[0]
    # Align _snap rows to features_full rows by goal_id + period_id
    for i, fidx in enumerate(snap_idx):
        gid = int(features_full.iloc[fidx]["goal_id"])
        pid = int(features_full.iloc[fidx]["period_id"])
        s_rows = _snap[(_snap["goal_id"] == gid) & (_snap["period_id"] == pid)]
        if len(s_rows) > 0:
            snap_row_idx = s_rows.index[0]
            if valid_snap[snap_row_idx]:
                y_resid_full[fidx] = float(y_llm_snap[snap_row_idx] - eng_full[fidx])

    # ── Step 3: Heteroscedastic alpha array ───────────────────────────────────
    alpha_arr = np.where(_snap_mask_full, NOISE_LLM, NOISE_ENG)

    print(f"\n  {dim}:")
    print(f"    LLM snapshot mean  = {y_llm_snap[valid_snap].mean():.3f}")
    print(f"    Engineered mean    = {eng_full.mean():.3f}")
    print(f"    Residual at snaps  = {y_resid_full[_snap_mask_full].mean():+.3f}  "
          f"std={y_resid_full[_snap_mask_full].std():.3f}")
    print(f"    Training rows      : 840 total  "
          f"({_snap_mask_full.sum()} LLM-labelled, "
          f"{(~_snap_mask_full).sum()} engineered-prior)")

    gp_dim = GaussianProcessRegressor(
        kernel=build_gp_kernel(),
        alpha=alpha_arr,       # per-sample noise — heteroscedastic
        normalize_y=True,
    ...
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        gp_dim.fit(X_all_periods, y_resid_full)

    dim_gps[dim] = gp_dim
    print(f"    Kernel: {gp_dim.kernel_}")

print(f"\n  All 4 GPs trained on 840 rows: {list(dim_gps.keys())}")
...
# Per-period R/C/I for all 840 rows via GP + engineered blend.
# Each dim GP was trained on (LLM - engineered) residuals at p6/p12/p18.
# Prediction = engineered_baseline + GP_residual, confidence-weighted.
# Falls back to pure engineered when GP uncertainty is high.
print("  Computing per-period R/C/I via GP + engineered blend for all 840 rows...")
...
_eng_full = {
    "relevance" : full_rel_eng,
    "coherence" : full_coh_eng,
    "integrity" : full_int_eng,
}

for dim in ["relevance", "coherence", "integrity"]:
    eng = _eng_full[dim]
    if dim not in dim_gps:
        full_preds[dim] = np.clip(eng, 0, 1)
        print(f"    {dim}: GP not available, using engineered only")
        continue

    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        gp_resid_all, gp_std_dim = dim_gps[dim].predict(X_all_periods, return_std=True)

    gp_pred   = np.clip(eng + gp_resid_all, 0, 1)
    gp_conf   = 1.0 / (1.0 + gp_std_dim * GP_UNCERTAINTY_SCALE)
    blended   = np.clip(gp_conf * gp_pred + (1.0 - gp_conf) * eng, 0, 1)
    full_preds[dim] = blended

    print(f"    {dim}: eng={eng.mean():.3f}  gp={gp_pred.mean():.3f}  "
          f"blended={blended.mean():.3f}  avg_conf={gp_conf.mean():.3f}")
...
with open("gp_config_poc.json", "w") as f:
    json.dump({
        "method": "CoherenceEngine_v10_MultiGP",
        "kernels": {d: str(dim_gps[d].kernel_) for d in dim_gps},
