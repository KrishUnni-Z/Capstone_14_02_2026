...
def load_artefacts():
    ...
    # Handle both old single GP (v9) and new dict of 4 GPs (v10)
    if isinstance(gp_data, dict):
        gp      = gp_data.get("attainability")
        gp_dict = gp_data
    else:
        gp      = gp_data
        gp_dict = {"attainability": gp_data}
    ...
    with open(os.path.join(_s2_root, "gp_config_poc.json")) as f:
        cfg = json.load(f)
    return gp, iso, scaler, cfg, gp_dict
...
def score_goal(goal_row, rule_row, verbose=True, current_period=18):
    gp, iso_scalers, feat_scaler, cfg, gp_dict = load_artefacts()
    ...
    def blend_dim(dim, rule_val):
        ...
        # GP prediction for this dimension (if available)
        gp_pred_val = None
        if dim in gp_dict and gp_dict[dim] is not None:
            try:
                with warnings.catch_warnings():
                    warnings.simplefilter("ignore")
                    _gp_r, _gp_s = gp_dict[dim].predict(X_scaled, return_std=True)
                gp_pred_val  = float(np.clip(_gp_r[0], 0, 1))
                gp_pred_conf = float(1.0 / (1.0 + _gp_s[0] * cfg.get("gp_uncertainty_scale", 50.0)))
            except Exception:
                gp_pred_val  = None
                gp_pred_conf = 0.0

        if len(ok) < 2 and gp_pred_val is None:
            return float(np.clip(rule_val, 0, 1)), {
                "rule_weight": 1.0, "llm_weight": 0.0,
                "gp_weight": 0.0, "variance": 0.0, "fallback": True,
            }

        arr = np.array(ok) if ok else np.array([rule_val])
        lm  = float(arr.mean())
        ...
        # Add GP as third signal if available
        if gp_pred_val is not None:
            wg   = gp_pred_conf * 0.4     # GP gets up to 40% weight at full confidence
            tot  = wr + wl + wg
            wr  /= tot; wl /= tot; wg /= tot
            blended = float(np.clip(wr * rule_val + wl * lm + wg * gp_pred_val, 0, 1))
            return blended, {
                "rule_weight": round(wr, 3),
                "llm_weight" : round(wl, 3),
                "gp_weight"  : round(wg, 3),
                "gp_pred"    : round(gp_pred_val, 3),
        ...
        return float(np.clip(wr * rule_val + wl * lm, 0, 1)), {
            ...
            "gp_weight": 0.0, "variance": round(var, 4),
            "llm_mean": round(lm, 3), "fallback": False,
        }
