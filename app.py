...
def load_data():
    ...
    def _is_composite(df):
        ...
        has_score = _has(cols, "composite", "composite_score", "composite_adjusted",
    ...
    if fwd is not None and "goal_id" in fwd.columns:
        ...
        if len(_frow) > 0:
            ...
            if _hist_df is not None:
                _ccol = "composite_adjusted" if "composite_adjusted" in _hist_df.columns else "composite"
...
# ═════════════════════════════════════════════════════════════════════════════
    ...
    def _prepare_goal_scores(df):
        ...
        if "composite" not in df.columns:
            for alt in ["composite_adjusted", "composite_score", "overall", "overall_score", "final_score", "score"]:
    ...
    with tab1:
        ...
        if ts is not None and len(ts) > 0:
            ...
            if delta_df is not None and 'composite_delta' in delta_df.columns:
                ...
                fig_delta = px.bar(
                    delta_df.sort_values("composite_delta"),
                    x="composite_delta",
                    y=[f"G{int(g)}" for g in delta_df['goal_id']],
                    orientation="h",
    ...
    with tab5:
        ...
        if fwd is not None and len(fwd) > 0:
            ...
            with col_b:
                # Always show composite MAE as the primary benchmark
                if 'composite_error_p24' in fwd.columns:
                    _comp_mae = fwd['composite_error_p24'].dropna().mean()
                elif 'composite_p6' in fwd.columns and 'composite_adjusted' in fwd.columns:
                    _comp_mae = (fwd['composite_p6'] - fwd['composite_adjusted']).abs().mean()
                else:
                    _comp_mae = None

                if _comp_mae is not None:
                    st.metric("Composite MAE (p18→p24)", f"{_comp_mae:.4f}")
                    st.metric("Benchmark",               "0.272",
                              help="Forward projection MAE defined as the pipeline benchmark.")
                if 'attain_error_p24' in fwd.columns:
                    st.metric("Attainability MAE (p18→p24)", f"{fwd['attain_error_p24'].dropna().mean():.4f}",
                              help="Secondary metric — attainability only, against held-out p24 ground truth.")
            ...
            if 'actual_composite_p24' in fwd.columns:
                ...
                val_df   = fwd[val_cols].sort_values('composite_error_p24').round(3) \
                    if 'composite_error_p24' in fwd.columns else fwd[val_cols].round(3)
